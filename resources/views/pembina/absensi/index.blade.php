            ...
            <table class="table table-hover align-middle text-center">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="text-start">
                            <div class="d-flex align-items-center">
                                ...
                                <img src="{{ asset('images/profile/' . $user->profile->foto) }}"
                                    alt="Foto {{ $user->name }}" class="rounded-circle me-2"
                        ...
                        <td>
                            <input type="checkbox" name="absensi[{{ $user->id }}]" value="hadir"
                                class="form-check-input status-checkbox" data-user="{{ $user->id }}">
                        </td>
                        <td>
                            <input type="text" name="keterangan[{{ $user->id }}]" class="form-control form-control-sm"
                                placeholder="Alasan Izin">
                        </td>
                        <td>
                            <input type="checkbox" name="absensi[{{ $user->id }}]" value="alfa"
                                class="form-check-input status-checkbox" data-user="{{ $user->id }}">
                        </td>
                    </tr>
                    @empty
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        userIds.forEach(userId => {
            const hadir = document.querySelector(
                `.status-checkbox[data-user="${userId}"][value="hadir"]`);
            const alfa = document.querySelector(
            `.status-checkbox[data-user="${userId}"][value="alfa"]`);
